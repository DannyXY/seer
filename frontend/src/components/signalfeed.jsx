...
// Honest data provenance: signals sourced from the deterministic fallback
// provider are labelled as simulated so live provider data is never confused
// with sample data.
const SOURCE_BADGES = {
  nansen: { label: "Nansen live", c: "var(--c-opp)", bg: "var(--c-opp-wash)" },
  defillama: { label: "DefiLlama live", c: "var(--c-opp)", bg: "var(--c-opp-wash)" },
  mock: { label: "Simulated", c: "var(--ink-2)", bg: "var(--card-2)" },
};

function SourceBadge({ source }) {
  const b = SOURCE_BADGES[String(source || "").toLowerCase()];
  if (!b) return null;
  return (
    <span className="badge" style={{ color: b.c, background: b.bg }} title={
      b.label === "Simulated"
        ? "Generated by the deterministic fallback provider - live provider data was unavailable for this signal."
        : "Sourced from a live data provider."
    }>{b.label}</span>
  );
}
