...
The current `NansenProvider` wires Nansen's `portfolio/defi-holdings` endpoint for wallet positions and wallet profile summaries. Protocol TVL/APY condition checks can use DeFiLlama through `DEFILLAMA_ENABLED=true`, falling back to mock data if unavailable. Smart-money, token-flow, transaction, and Nansen-native protocol metric methods still fall back through `ProviderRegistry` until their live response schemas are mapped. `MockProvider` remains the deterministic Mantle-oriented demo fallback.
